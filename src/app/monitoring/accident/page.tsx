/**
 * @file src/app/monitoring/accident/page.tsx
 * @description CTQ 사고성 모니터링 페이지
 *
 * 초보자 가이드:
...
export default function AccidentPage() {
  const [monitorInterval, setMonitorInterval] = usePersistedState("ctq-accident-monitor-interval", 30000);
  const [rollingInterval, setRollingInterval] = usePersistedState("ctq-accident-rolling-interval", 10000);
  const [rollingEnabled, setRollingEnabled] = usePersistedState("ctq-accident-rolling-enabled", true);
        ...
        <div className="flex items-center justify-between max-w-[1920px] mx-auto">
          ...
          <div className="flex flex-col items-end gap-1">
            <div className="flex items-center gap-4 text-xs text-gray-400">
              ...
              <span><span className="text-red-400 font-bold">{t("grade.a") as string}</span>: HIPOT 1+, BURN-IN/ATE 2+</span>
              <span><span className="text-orange-400 font-bold">{t("grade.b") as string}</span>: BURN-IN/ATE 1</span>
            </div>
        ...
        <div className="flex items-center justify-between max-w-[1920px] mx-auto">
          ...
          <div className="flex items-center gap-4">
            {data && data.lines.length > 0 && (
              <>
                <SummaryBadge label={t("pages.accident.gradeALabel") as string} count={aCount} color="bg-red-600" />
