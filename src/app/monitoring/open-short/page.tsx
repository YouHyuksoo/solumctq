/**
 * @file src/app/monitoring/open-short/page.tsx
 * @description 공용부품 Open/Short 모니터링 페이지 (ICT 공정)
 *
 * 초보자 가이드:
...
export default function OpenShortPage() {
  ...
  const { data, error, loading } = useOpenShort(monitorInterval, selectedLines, isInitialized);
        ...
        <div className="flex items-center justify-between max-w-[1920px] mx-auto">
          ...
          <div className="flex flex-col items-end gap-1">
            <div className="flex items-center gap-4 text-xs text-gray-400">
              ...
              <span><span className="text-orange-400 font-bold">{t("grade.b") as string}</span>: {t("pages.openShort.title") as string} — {t("table.sameComponent") as string} + {t("table.sameBadCode") as string} {t("table.dailyNg") as string} 2+</span>
            </div>
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
              {visibleLines.map((line) => (
                <OpenShortLineCard key={line.lineCode} line={line} />
              ))}
            </div>
            {totalPages > 1 && (
